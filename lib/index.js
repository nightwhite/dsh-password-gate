 ...
 * resolved by the patch row); when neither is set a random one is generated
 * and printed once at startup, Jupyter-style.
...
var GatedWebServer = class extends WebServer {
	...
	constructor(ctx, config) {
		...
		if (this.password === null) {
			...
			console.log(`dsh-password-gate: generated web access password: ${this.password} (set DSH_WEB_PASSWORD or pass --password to choose one)`);
		}
	}

	/** Wrap the bound server's listeners: check auth, then forward. */
	async [Service.init]() {
